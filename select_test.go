...
var selectCaseMap = map[string]string{
	...
	"select occupy from ark where process_id= 1":                                                             `{"query" : {"bool" : {"must" : [{"term" : {"process_id" : "1"}}]}},"from" : 0,"size" : 1}`,
	"select occupy from ark where (process_id= 1)":                                                           `{"query" : {"bool" : {"must" : [{"term" : {"process_id" : "1"}}]}},"from" : 0,"size" : 1}`,
	"select occupy from ark where ((process_id= 1))":                                                         `{"query" : {"bool" : {"must" : [{"term" : {"process_id" : "1"}}]}},"from" : 0,"size" : 1}`,
	"select occupy from ark where (process_id = 1 and status=1)":                                             `{"query" : {"bool" : {"must" : [{"term" : {"process_id" : "1"}},{"term" : {"status" : "1"}}]}},"from" : 0,"size" : 1}`,
	...
	"select occupy from ark_callcenter where process_id != 1":                                                `{"query" : {"bool" : {"must" : [{"bool" : {"must_not" : [{"term" : {"process_id" : "1"}}]}}]}},"from" : 0,"size" : 1}`,
	"select occupy from ark_callcenter where process_id = 0 and status= 1 and channel = 4":                   `{"query" : {"bool" : {"must" : [{"term" : {"process_id" : "0"}},{"term" : {"status" : "1"}},{"term" : {"channel" : "4"}}]}},"from" : 0,"size" : 1}`,
	...
	"select * from ark_callcenter where process_id > 1 and status = 1":                                       `{"query" : {"bool" : {"must" : [{"range" : {"process_id" : {"gt" : "1"}}},{"term" : {"status" : "1"}}]}},"from" : 0,"size" : 1}`,
	"select * from ark_callcenter where create_time between '2015-01-01T00:00:00+0800' and '2017-01-01T00:00:00+0800' and process_id = 0 and status >= 1 and content = '三个男人' and phone = '15810324322'": `{"query" : {"bool" : {"must" : [{"range" : {"create_time" : {"from" : "2015-01-01T00:00:00+0800", "to" : "2017-01-01T00:00:00+0800"}}},{"term" : {"process_id" : "0"}},{"range" : {"status" : {"from" : "1"}}},{"term" : {"content" : "三个男人"}},{"term" : {"phone" : "15810324322"}}]}},"from" : 0,"size" : 1}`,
	"select * from ark_callcenter where id > 1 or process_id = 0":                                      `{"query" : {"bool" : {"should" : [{"range" : {"id" : {"gt" : "1"}}},{"term" : {"process_id" : "0"}}]}},"from" : 0,"size" : 1}`,
	"select * from ark where id > 1 and d = 1 or process_id = 0 and x = 2":                             `{"query" : {"bool" : {"should" : [{"bool" : {"must" : [{"range" : {"id" : {"gt" : "1"}}},{"term" : {"d" : "1"}}]}},{"bool" : {"must" : [{"term" : {"process_id" : "0"}},{"term" : {"x" : "2"}}]}}]}},"from" : 0,"size" : 1}`,
	...
	"select * from ark where (id > 1 and d = 1) or (c=1)":                                              `{"query" : {"bool" : {"should" : [{"bool" : {"must" : [{"range" : {"id" : {"gt" : "1"}}},{"term" : {"d" : "1"}}]}},{"term" : {"c" : "1"}}]}},"from" : 0,"size" : 1}`,
	"select * from ark where id > 1 or (process_id = 0)":                                               `{"query" : {"bool" : {"should" : [{"range" : {"id" : {"gt" : "1"}}},{"term" : {"process_id" : "0"}}]}},"from" : 0,"size" : 1}`,
	"select * from ark where id in (1,2,3,4)":                                                          `{"query" : {"bool" : {"must" : [{"terms" : {"id" : [1, 2, 3, 4]}}]}},"from" : 0,"size" : 1}`,
	"select * from ark where id in ('232', '323') and content = 'aaaa'":                                `{"query" : {"bool" : {"must" : [{"terms" : {"id" : ["232", "323"]}},{"term" : {"content" : "aaaa"}}]}},"from" : 0,"size" : 1}`,
	...
	"select x from ark where a like '%a%'":                                                             `{"query" : {"bool" : {"must" : [{"match" : {"a" : {"query" : "a"}}}]}},"from" : 0,"size" : 1}`,
	...
	//	"select * from ark where a like group_concat('%', 'abc', '%')":                                                                                                                                       `{"query" : {"bool" : {"must" : [{"term" : {"a" : {"query" : "abc"}}}]}},"from" : 0,"size" : 1}`,
	"select * from `order`.abcd where `by` = 1":                                `{"query" : {"bool" : {"must" : [{"term" : {"by" : "1"}}]}},"from" : 0,"size" : 1}`,
	"select * from ark where id not like '%aaa%'":                              `{"query" : {"bool" : {"must" : [{"bool" : {"must_not" : {"match" : {"id" : {"query" : "aaa"}}}}}]}},"from" : 0,"size" : 1}`,
	...
	"select * from a where id is null":                                         `{"query" : {"bool" : {"must" : [{"bool" : {"must_not" : [{"exists":{"field":"id"}}]}}]}},"from" : 0,"size" : 1}`,
	"select * from a where id is not null":                                     `{"query" : {"bool" : {"must" : [{"bool" : {"must" : [{"exists":{"field":"id"}}]}}]}},"from" : 0,"size" : 1}`,
	...
	"select * from aaa where  a= 1 and multi_match(query='this is a test', fields=(title,title.origin))":                      `{"query" : {"bool" : {"must" : [{"term" : {"a" : "1"}},{"multi_match" : {"query" : "this is a test", "fields" : ["title","title.origin"]}}]}},"from" : 0,"size" : 1}`,
	"select * from aaa where  a= 1 and multi_match(query='this is a test', fields=(title,title.origin), type=phrase)":         `{"query" : {"bool" : {"must" : [{"term" : {"a" : "1"}},{"multi_match" : {"query" : "this is a test", "fields" : ["title","title.origin"], "type" : "phrase"}}]}},"from" : 0,"size" : 1}`,
	"SELECT `id`, `name` FROM `student` WHERE (`name` LIKE '%aaa!_bbb!!ccc%' ESCAPE '!') ORDER BY `updateTime` DESC LIMIT 10": `{"query" : {"bool" : {"must" : [{"match" : {"name" : {"query" : "aaa_bbb!ccc"}}}]}},"from" : 0,"size" : 10,"sort" : [{"updateTime": "desc"}]}`,
	"SELECT `id`, `name` FROM `student` WHERE (`name` LIKE '%aaa!_bbb!!ccc%') ORDER BY `updateTime` DESC LIMIT 10":            `{"query" : {"bool" : {"must" : [{"match" : {"name" : {"query" : "aaa!_bbb!!ccc"}}}]}},"from" : 0,"size" : 10,"sort" : [{"updateTime": "desc"}]}`,
}
